...
public class HTTPClientInitializer extends ChannelInitializer<SocketChannel> {
    ...
    public HTTPClientInitializer(SSLEngine sslEngine, boolean httpTraceLogEnabled, boolean isChunkedDisabled) {
    ...
    protected void initChannel(SocketChannel ch) throws Exception {
        ...
        ch.pipeline().addLast("compressor", new CustomHttpContentCompressor(isChunkedDisabled));
        ch.pipeline().addLast("decoder", new HttpResponseDecoder());
        ch.pipeline().addLast("encoder", new HttpRequestEncoder());
        if (!isChunkedDisabled) {
